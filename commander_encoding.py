...
def at_most_one(a: list, start: int):
    number_of_group = math.floor(math.sqrt(len(a)))
    group_len = len(a) // number_of_group
    if group_len * number_of_group < len(a):
        number_of_group += 1
    groups = []
    new_var = []
    exprs = []
    for i in range(number_of_group):
        groups.append(a[i * group_len:(i + 1) * group_len])
        new_var.append(start)
        start += 1

    for i in range(len(groups)):
        for e in groups[i]:
            exprs.append([-e, new_var[i]])

    for i in range(len(groups)):
        exprs+= binomial_at_most_one(groups[i])
        groups[i].append(-new_var[i])
        exprs.append(groups[i])
    exprs += binomial_at_most_one(new_var)
    return exprs, start
